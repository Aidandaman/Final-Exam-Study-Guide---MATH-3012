...
Then, the quantity we desire to find is the number of solutions which violate any of the 3 conditions: $N(\overline{c_1c_2c_3}) = |A| - S_1+S_2-S_3$.
    \begin{itemize}
        ...
        \item $S_1$, the number of elements where 1 condition is satisfied is $\binom{8}{2}+\binom{7}{2}+\binom{6}{2}$. Thus our total is:
...
\subsubsection{Determining items that satisfy exactly $i$ conditions.}
\label{Module 3.2.1}

\textcolor{navy}{\textbf{Theorem}}: The number $E_i$ denotes the number of elements in $A$ satisfying \textbf{EXACTLY} $i$ conditions $c_1,...,c_n$:
    $$E_k=S_k-\binom{k+1}{1}S_{k+1}+\binom{k+2}{3}S_{k+2}-\space...\space+(-1)^{n-k}\binom{n}{n-k}S_n$$

\begin{itemize}
    \item Remember that $S_i$ has no permanent definition. It really just represents the way to satisfy $i$ conditions by each way with no checks for over-counting. We need to be smart and change $S_i$ to be whatever the question is asking for.
\end{itemize}
...
\textcolor{red}{\textbf{Definition}}: A \underline{Rook Polynomial} is the way to arrange rooks on some chessboard of $C$. It is expressed as $r_k(C)$. Remember that $r_0(C)=1$ always. Now, if we have a $n \times x$ regular, monochrome board our formula for our rook polynomial is:
    $$r(C_{n\times n})=\sum^n_{k=0}r_kx^k, \space \text{where } r_k=\binom{n}{k}\binom{n}{k}k!$$
...
\textcolor{red}{\textbf{TRY IT}}: Find the rook polynomial of the chessboard below:
    ...
    \end{center}
\begin{itemize}
    \item $r_0(C_2)=0$ by convention.
    \item $r_1(C_2)=4$: we can place a single rook 4 ways, 1 in each square.
    \item $r_2(C_2)=2$: to place 2 rooks, we must place both in a diagonal.
    \item $r_3(C_3)=0$: we don't have enough diagonal squares to place 3.
        $$r(C)=0+4x+2x^2+0\cdot x^3$$
\end{itemize}
